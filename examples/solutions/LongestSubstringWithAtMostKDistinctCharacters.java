...
public class LongestSubstringWithAtMostKDistinctCharacters {
    public int lengthOfLongestSubstringKDistinct(String s, int k) {
        ...
        while (r < s.length()) {
            while (r < s.length() &&
                   (existingCharsCounter.size() < k || existingCharsCounter.containsKey(chars[r]))) {
            ...
            while (existingCharsCounter.size() == k) {
                existingCharsCounter.compute(chars[l], (ignored, v) -> --v == 0 ? null : v);
                l++;
            }
        }
        return longest;
